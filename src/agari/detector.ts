...
import type { AgariConfig, Mentsu, YakuName } from "@/types/yaku";
import { getAppliableRules } from "../yaku";
import { decomposeTehai } from "./structure";
 ...
 * @param furo - Optional list of fixed melds (open calls or Kans).
...
export function detectAgari(
	...
	const structures = decomposeTehai(haiCounts, agariHai, furo);

	if (structures.length === 0) {
		throw new Error("Invalid hand structure (not 4 mentsu + 1 jantou)");
	}
