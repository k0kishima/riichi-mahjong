...
import type { ShantenNumber } from "@/types/shanten";
...
function updateMinShanten(state: ShantenState): void {
	let shanten =
		8 - state.numberMentsu * 2 - state.numberTatsu - state.numberToitsu;
	...
	if (state.numberToitsu) {
		...
		if (
			(state.flagFourCopies | state.flagIsolatedTiles) ===
			state.flagFourCopies
		) {
			shanten += 1;
		}
	}
...
function processThreeTiles(
	...
	if (
		normalizedIndex < 7 &&
		state.tiles[index + 2] >= 2 &&
		state.tiles[index + 1] >= 2
	) {
		increaseSequence(state, index);
		increaseSequence(state, index);
		searchBestComposition(state, index);
 ...
 */
export function calculateShantenForRegularHand(
	haiCounts: HaiCounts,
): ShantenNumber {
