...
import { calculateMachi } from "../src/machi";

describe("calculateMachi", () => {
	it("should calculate waits for a simple ryanmen", () => {
		...
		const machi = calculateMachi(hand);
		expect(machi).toEqual([0, 3]); // 1m, 4m
	});
	...
	it("should calculate waits for sanmenchan", () => {
		...
		const machi = calculateMachi(hand);
		expect(machi).toEqual([0, 3, 6]); // 1m, 4m, 7m
	});
	...
	it("should calculate waits for shanpon (dual pon wait)", () => {
		...
		const machi = calculateMachi(hand);
		expect(machi.sort()).toEqual([0, 28].sort()); // 1m, 2z
	});
	...
	it("should calculate waits for nobetan (extended wait)", () => {
		...
		const machi = calculateMachi(hand);
		expect(machi.sort()).toEqual([1, 4].sort()); // 2m, 5m
	});
	...
	it("should throw error for invalid tile count", () => {
		...
		expect(() => calculateMachi(hand)).toThrow("Invalid hai count");
	});
});
